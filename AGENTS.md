...
- 标记为 `metadata.status: retired`，或描述明确声明“已退役兼容”的 Skill，不参与“至少 1% 适用”检查和主动路由；仅当用户或历史流程明确引用其名称时加载，并严格执行其兼容回退行为。
...
- **开发模式触发：** 以下工程影响型写入开始前，必须确定当前请求使用 `/grill-me` 轻量流程还是 `superpowers` 完整流程：新增功能、Bug 修复、重构和行为变更；源码与测试修改；构建、依赖、配置、数据库结构和迁移修改；API、架构、安全、部署及 CI/CD 变更；`AGENTS.md`、`SKILL.md`、工具路由等会改变 Agent 行为的规则修改。
- **无需选择模式：** 纯解释、只读分析、代码阅读、状态查询和报告；不改变语义的拼写、排版及格式修正；用户已明确来源、目标和策略的机械复制或同步；自动生成过程文档的移动、归档和清理。无法确定是否影响工程或 Agent 行为时，必须先询问模式，不得自行按低风险处理。
- 用户已在当前请求中明确指定开发模式时直接采用，不重复确认；用户未指定且请求命中开发模式触发范围时，必须暂停实现并询问用户选择，不得代替用户决定。
- 开发模式只对当前请求有效。不触发开发模式不代表扩大授权；删除、覆盖、外部写入等操作仍须遵守授权、工作区保护和破坏性操作规则。
- **方案推敲与决策确认原则：** 无论使用何种开发模式，在展开任何功能实现、架构方案、技术选型或代码编写之前，**必须先使用 `/grill-me`（`grilling` 极限推敲）向哥哥提问，压力测试关键决策并由哥哥确认方案；只有在哥哥明确确认方案之后，方可进入后续实施阶段**。
- **UI 设计只出 HTML 示例原则（视觉与代码绝对隔离）：** UI 视觉与交互设计阶段（包含页面原型、视觉重构、组件样式、交互与动效设计）**只出独立的 HTML 示例供预览体验，严禁修改代码；除非有哥哥明确告知“按照设计修改代码”，在此之前绝对不得触碰或改动正式工程业务代码**。
...
- **`/grill-me` 轻量流程：** 方案制定与编码实施之前，必须使用 `grilling` 对需求、关键决策、架构选型与边界情况进行极限推敲，以带推荐答案的选项树形式向哥哥提问；**方案获哥哥明确确认后直接实施**，不强制调用 `brainstorming`、`writing-plans`、`executing-plans`、`test-driven-development`、`requesting-code-review` 或 `receiving-code-review`。
- **`superpowers` 完整流程：** 创造性实现或重要行为变更在方案制定与编码实施之前，同样必须先使用 `/grill-me`（`grilling`）对方案意图与关键决策进行推敲收敛并**获得哥哥明确确认**；确认后使用 `writing-plans` 制定实施计划，计划获批后普通任务直接在当前会话内联执行计划，需要分批检查点时使用 `executing-plans`，用户提供 YAML 工作流或明确要求多角色协作时使用 `workflow-runner`。
...
- **设计工程与高级动效：** 涉及界面质感打磨、动效构建、手势交互与组件库选型时按职责调用：
  - **设计只出 HTML 示例门禁（严禁修改代码）：** 任何涉及 UI、视觉、交互、布局或动效的设计任务，**在设计阶段只出独立的 HTML 示例，严禁修改业务代码**。示例统一保存在 `docs/.generated/examples/<任务名>/` 或项目指定的预览路径中，并向哥哥展示可验证、可交互的完整浏览器预览体验。
...
- **反过度工程与极简实现（全局强制生效）：** 极简主义与 YAGNI 是所有开发、修改、重构过程中的全流程常驻硬性约束，绝非仅在用户特定提示时才启用的选修项：
  - `ponytail`：作为实现阶段的强制收敛准则，严格遵循 7 级决策阶梯（YAGNI ➔ 现有复用 ➔ 标准库 ➔ 平台原生 ➔ 一行解决 ➔ 最少实现），绝不编写未经要求的抽象、单实现接口、无外部调用的孤立方法或空壳别名；
...
`grilling`（`/grill-me`）是方案推敲与决策确认的核心武器，在进入具体编码或方案制定前向哥哥发起决策树提问，确保方案完全受控；`systematic-debugging` 仅在出现 Bug、测试失败、构建失败、性能问题或异常行为时触发。

### 3.3 质量硬门禁

- **模式选择门禁：** 命中工程影响型写入范围的请求，在用户指定或选择开发模式前不得开始实现。
- **方案确认门禁（Pre-Implementation Grilling Gate）：** **实施之前用 `/grill-me` 让我确认方案，确认之后再实施**。任何涉及功能实现、技术选型、架构调整、新增功能或重要行为变更的任务，编码实施前必须先使用 `grill-me`（`grilling`）向哥哥提问、暴露假设与边界并呈现推荐选项，直到哥哥明确确认方案。**方案未经哥哥明确确认，严禁开始编写实现代码与落地变更**。
- **UI 设计只出 HTML 示例门禁（Design HTML Isolation Gate）：** **UI 设计只出 HTML 示例，严禁修改代码**。任何涉及 UI、视觉、交互与动效的设计任务，在设计阶段只出独立 HTML 示例供浏览器预览。**严禁修改业务代码，除非有哥哥明确告知“按照设计修改代码”**。
- **极简主义与 YAGNI 门禁：** 任何编码修改写入与交付前，必须核查改动是否为达成目标的最少必要代码。严禁添加任何无外部调用方的函数/属性、未经要求的别名包装、单实现接口与推测性抽象；一旦发现无实际调用，必须立即删除，绝不留存。
...
```text
/grill-me：实施前 grill-me 极限推敲 ➔ 哥哥明确确认方案 ➔ [若涉 UI/视觉设计] 仅出独立 HTML 示例 ➔ [若涉 UI/视觉设计] 哥哥明确告知按照设计修改代码 ➔ 直接实施 ➔ 异常时系统化调试 ➔ 最小必要验证 ➔ 交付
superpowers：实施前 grill-me 极限推敲 ➔ 哥哥明确确认方案 ➔ [若涉 UI/视觉设计] 仅出独立 HTML 示例 ➔ [若涉 UI/视觉设计] 哥哥明确告知按照设计修改代码 ➔ 制定实施计划 ➔ 计划获批 ➔ TDD 实现与局部测试 ➔ Code Review ➔ 反馈验证与复审 ➔ 完整验证 ➔ 交付
```
...
- 针对 UI 视觉重构、样式美化、组件原型、交互与动效类设计请求，授权默认严格限定在生成与迭代独立 HTML 示例，绝对不包含修改正式业务代码的授权；**只有当哥哥明确告知“按照设计修改代码”时，才获得修改正式工程代码的授权**。
...
- 技术方案、架构选型或关键决策未经 `/grill-me`（`grilling`）推敲并获得哥哥明确确认；
- 处于 UI 设计阶段或仅完成 HTML 示例，尚未收到哥哥明确告知“按照设计修改代码”却准备修改正式业务代码；
- 关键需求存在会显著改变最终结果的歧义；
- 计划依赖缺失、环境不可用或验证反复失败；
...
- **正式文档：** 长期维护、用户明确要求保留或属于项目事实源的文档统一放在 `docs/` 的常规分类目录中，并允许纳入 Git；严禁散落在项目根目录。
- **过程文档：** Agent 在执行任务时自动生成的需求分析、设计规格、实施计划、任务清单和阶段报告统一放在 `docs/.generated/`，默认由 Git 忽略，不得擅自提交。
- **文档提升：** 只有用户明确要求纳入 Git 时，才将过程文档整理并移动到 `docs/` 的常规分类目录；不得仅通过强制暂存被忽略文件绕过此规则。
- **文件命名规范：**
  - 文档名称应简洁明了，具有描述性（例如：`2026-08-25_UI_改色与排版优化_需求文档.md`）。
  - 文件名应包含日期（`YYYY-MM-DD` 格式）、核心主题和文档类型。
  - 避免使用不必要的空格或特殊字符，使用下划线 `_` 或中划线 `-` 分隔单词。
- **正式文档分类：** 可以在 `docs/` 目录下创建子目录，例如：
  - `docs/requirements/` - 正式需求文档
  - `docs/design/` - 正式架构与设计决策
  - `docs/reference/` - 项目参考资料
  - `docs/guides/` - 长期维护的使用与开发指南

## 12. 极简主义与 YAGNI 开发规范 (Minimalism & YAGNI Mandate)

开发实现过程中，必须将极简主义（Minimalism）与 YAGNI（You Aren't Gonna Need It）作为无条件遵循的核心铁律，杜绝任何形式的过度设计与代码膨胀：

### 12.1 坚决杜绝推测性代码 (No Speculative Code)
- **拒绝脑补需求：** 严禁添加任何“以后可能会用到”、“顺手做一下”、“提供额外便利”的方法、参数、重载、配置或扩展点。只有当用户在当前指令中明确要求，或现有调用链确实必需时，才允许编写对应逻辑。
- **拒绝冗余抽象：** 严禁编写单一实现的接口（Single-implementation Interface）、只有一层转发的空壳抽象类或未经证明的策略工厂。
- **拒绝过度参数化：** 不添加没有真实调用场景的默认参数、标志位或泛型约束。

### 12.2 严禁无调用方的死代码与空壳别名 (No Unused Code & Empty Aliases)
- **禁止空壳别名方法：** 严禁为了“看起来更语义化”或“方法同义词”而新增只做单层转发的包装方法（例如严禁编写类似 `fun resetState() { clearAll() }` 这种没有独立业务价值且在外部已有成熟调用方的空壳方法）。
- **零孤儿函数/属性：** 新增的每个公开或私有方法、字段、变量，必须在本次改动中有直接的实际调用方；一旦发现无外部调用，必须就地删除，绝不留存。
- **优先就地复用现有体系：** 当项目中已有既定方法（如 `clearAll()`）承担相应职责时，应直接在已有方法中做必要补充或直接复用，坚决杜绝新增平行方法造成概念混淆。

### 12.3 7 级极简决策阶梯 (The 7-Step Ladder)
在编写或修改任何一段代码时，强制按以下阶梯自上而下收敛，坚决停在能满足需求的最浅层级：
1. **第 0 级（YAGNI - 不做）：** 这段逻辑/方法不写，功能是否完全正常？如果是，直接不写；
2. **第 1 级（现有复用）：** 项目已有方法、扩展函数或组件能否直接搞定？有则直接调用已有代码；
3. **第 2 级（标准库）：** 语言标准库（Kotlin stdlib / Java 原生 API）能否解决？有则优先使用标准库；
4. **第 3 级（平台原生）：** Android / iOS / Web 平台 SDK 原生能力能否一行解决？优先原生能力，拒绝引入额外三方依赖或自定义封装；
5. **第 4 级（就地内联）：** 1~3 行能完成的简单逻辑，直接就地内联编写，严禁抽取为仅使用一次的私有方法或成员；
6. **第 5 级（最小实现）：** 必须新建函数时，只写达成目标的最少代码行数，拒绝多余的分层、防御性空检查或装饰器；
7. **第 6 级（抽象重构）：** 只有当完全相同的重复代码出现 3 次以上（Rule of Three）且具有稳定多态需求时，才允许进行抽象。

### 12.4 交付前 YAGNI 三问自检 (Pre-Delivery Self-Check)
每次编辑代码完毕、向用户汇报之前，必须执行自动化意识走查：
1. **调用方审计：** 本次改动新增的所有函数、属性、类，是否均有至少一处有效调用？是否存在孤立的新增符号？（有则必删！）
2. **包装度走查：** 是否新增了仅有一行调用的透传包装？是否给现有方法套了没有新逻辑的别名？（有则必删！）
3. **需求边界审查：** 本次 diff 中是否有任何一行代码超出了用户当前指令的最小必要集合？（有则剔除，恢复极简！）
