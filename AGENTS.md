...
- **完整流程门禁：** 实施计划批准、TDD 和代码审查门禁仅在 `superpowers` 完整流程中强制执行；`/grill-me` 轻量流程明确豁免这些强制链路。
...
```text
/grill-me：设计前 grill-me 极限推敲 ➔ 哥哥明确确认方案 ➔ 设计只出 HTML 示例 ➔ 哥哥明确告知按照设计修改代码 ➔ 直接实施 ➔ 异常时系统化调试 ➔ 最小必要验证 ➔ 交付
superpowers：设计前 grill-me 极限推敲 ➔ 哥哥明确确认方案 ➔ 设计只出 HTML 示例 ➔ 哥哥明确告知按照设计修改代码 ➔ 制定实施计划 ➔ 计划获批 ➔ TDD 实现与局部测试 ➔ Code Review ➔ 反馈验证与复审 ➔ 完整验证 ➔ 交付
```
