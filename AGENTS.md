...
- **设计前推敲与方案确认原则：** 无论使用何种开发模式，在展开任何功能设计、架构方案或技术选型之前，**必须先使用 `/grill-me`（`grilling` 极限推敲）向哥哥提问，压力测试关键决策并由哥哥确认方案；只有在哥哥明确确认方案之后，方可进入后续实施阶段**。
- **设计只出 HTML 示例原则（设计与代码绝对隔离）：** 设计阶段（包含页面原型、视觉重构、组件样式、交互与动效设计）**只出独立的 HTML 示例供预览体验，严禁修改代码；除非有哥哥明确告知“按照设计修改代码”，在此之前绝对不得触碰或改动正式工程业务代码**。
...
- **`/grill-me` 轻量流程：** 设计之前必须使用 `grilling` 对需求、关键决策、架构选型与边界情况进行极限推敲，以带推荐答案的选项树形式向哥哥提问；**方案获哥哥明确确认后直接实施**，不强制调用 `brainstorming`、`writing-plans`、`executing-plans`、`test-driven-development`、`requesting-code-review` 或 `receiving-code-review`。
- **`superpowers` 完整流程：** 创造性实现或重要行为变更在设计之前，同样必须先使用 `/grill-me`（`grilling`）对方案意图与关键决策进行推敲收敛并**获得哥哥明确确认**；确认后使用 `writing-plans` 制定实施计划，计划获批后普通任务直接在当前会话内联执行计划，需要分批检查点时使用 `executing-plans`，用户提供 YAML 工作流或明确要求多角色协作时使用 `workflow-runner`。
...
`grilling`（`/grill-me`）是设计前推敲与方案确认的核心武器，在进入方案设计前向哥哥发起决策树提问，确保方案完全受控；`systematic-debugging` 仅在出现 Bug、测试失败、构建失败、性能问题或异常行为时触发。

### 3.3 质量硬门禁

- **模式选择门禁：** 命中工程影响型写入范围的请求，在用户指定或选择开发模式前不得开始实现。
- **方案确认门禁（Pre-Design Grilling Gate）：** **设计之前用 `/grill-me` 让我确认方案，确认之后再实施**。任何涉及方案设计、技术选型、架构调整、新增功能或重要行为变更的任务，设计前必须先使用 `grill-me`（`grilling`）向哥哥提问、暴露假设与边界并呈现推荐选项，直到哥哥明确确认方案。**方案未经哥哥明确确认，严禁开始编写实现代码与落地变更**。
- **设计只出 HTML 示例门禁（Design HTML Isolation Gate）：** **设计只出 HTML 示例，严禁修改代码**。任何涉及 UI、视觉、交互与动效的设计任务，在设计阶段只出独立 HTML 示例供浏览器预览。**严禁修改业务代码，除非有哥哥明确告知“按照设计修改代码”**。
...
```text
/grill-me：设计前 grill-me 极限推敲 ➔ 哥哥明确确认方案 ➔ [若涉 UI/设计] 仅出独立 HTML 示例 ➔ [若涉 UI/设计] 哥哥明确告知按照设计修改代码 ➔ 直接实施 ➔ 异常时系统化调试 ➔ 最小必要验证 ➔ 交付
superpowers：设计前 grill-me 极限推敲 ➔ 哥哥明确确认方案 ➔ [若涉 UI/设计] 仅出独立 HTML 示例 ➔ [若涉 UI/设计] 哥哥明确告知按照设计修改代码 ➔ 制定实施计划 ➔ 计划获批 ➔ TDD 实现与局部测试 ➔ Code Review ➔ 反馈验证与复审 ➔ 完整验证 ➔ 交付
```
...
- 针对设计、视觉重构、样式美化、交互与动效类请求，授权默认严格限定在生成与迭代独立 HTML 示例，绝对不包含修改正式业务代码的授权；**只有当哥哥明确告知“按照设计修改代码”时，才获得修改正式工程代码的授权**。
...
- 设计方案、架构选型或关键决策未经 `/grill-me`（`grilling`）推敲并获得哥哥明确确认；
- 处于设计阶段或仅完成 HTML 示例，尚未收到哥哥明确告知“按照设计修改代码”却准备修改正式业务代码；
- 关键需求存在会显著改变最终结果的歧义；
- 计划依赖缺失、环境不可用或验证反复失败；
