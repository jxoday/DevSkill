...
description: Google official NowInAndroid architecture guidance and modern enterprise Android architecture patterns. Use when designing multi-module project structures (api/impl separation), offline-first data layers with Room, Hilt dependency injection, unidirectional data flow (UDF / ViewModel + UiState), and Gradle convention plugins (build-logic). Triggers on requests to structure Android architecture, create feature modules, implement repositories, or design data layers.
---
