...
- **macOS / Linux (Bash)**:
  ```bash
  bash -c 't=$(mktemp -d); git clone https://github.com/jxoday/DevSkill.git "$t" && mkdir -p ~/.gemini/config/skills && cp -r "$t"/*/ ~/.gemini/config/skills/ && rm -rf "$t" && echo "✅ DevSkill 已成功安装到 Antigravity / Gemini CLI！"'
  ```
